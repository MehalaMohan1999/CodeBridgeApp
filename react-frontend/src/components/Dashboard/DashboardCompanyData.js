...
export const CompanyData = (props) => {
  ...
  const get = async () => {
    ...
    const adminRole = currentCache.roles.find(
      (role) => role.role === "Staff" && role.position === "Admin",
    );

    if (
      adminRole &&
      adminRole.preferences &&
      adminRole.preferences.pastTabsAry
    ) {
      const pastTabsAry = adminRole.preferences.pastTabsAry;

      setRecentItems(
        pastTabsAry
          .filter((item) => item.mainMenu === 7)
          .slice(-3)
          .map((i) => ({
            text: i.label,
            subtext: "Notification",
      ...
      console.warn("Admin role or pastTabsAry not found in cache.");
    }
  };
