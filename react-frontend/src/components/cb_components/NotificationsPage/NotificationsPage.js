...
const NotificationsPage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("notifications");
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("notifications").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="mt-5">
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <NotificationsEditDialogComponent
