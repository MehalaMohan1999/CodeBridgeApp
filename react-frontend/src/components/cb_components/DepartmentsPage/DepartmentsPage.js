...
const DepartmentsPage = (props) => {
  ...
  const fetchPreferences = async () => {
    try {
      ...
      console.log("saved cahce: ", savedCache);
      const data = savedCache.results;
      const userRole = _.find(data.roles, { role: "Staff", position: "Admin" });

      if (userRole && userRole.preferences) {
        const departmentsSettings = _.get(
          userRole,
          "preferences.settings.departments",
        );

        if (departmentsSettings) {
          // Set the preferences using existing departments settings
          setPreferences(userRole.preferences);
          setSelectedHideFields(departmentsSettings.hiddenFields || []);
          setSelectedFilterFields(departmentsSettings.filterFields || []);
        } else {
          // If departments settings do not exist, initialize them
          console.warn("Departments settings not found, initializing...");
  ...
  const updatePreferences = async (newPreferences) => {
    try {
      ...
      const updatedDepartments = {
        ..._.get(settings, "departments", {}),
        hiddenFields: _.get(
          newPreferences,
          "hiddenFields",
          _.get(settings, "departments.hiddenFields", []),
        ),
        filterFields: _.get(
          newPreferences,
          "filterFields",
          _.get(settings, "departments.filterFields", []),
        ),
      };

      const updatedSettings = _.set(
        settings,
        "departments",
        updatedDepartments,
      );
